...
IM.Verify = function(node,stage){
	if(stage==0){
		node.find("*[data-widget-property=pre-state]").fadeOut(
			300,
			function() {
				node.addClass('pending-verification');
				node.find("*[data-widget-property=post-state]").fadeIn(
					300,
					function() {
...
IM.OptIn = function(node) {
	...
	node.find(".inline-processing").show(0);
			...
			IM.OptOutComplete = function(node){
				...
				node.find('*[data-widget-type=action-prompt]').each(function(){ HW.ActionPrompt.init( $(this) ); $(this).click( HW.ActionPrompt.reveal ); });
				//node.find("#opt-out").text("Enable SMS").attr("data-widget-name", nodeName+"-optin");
				IM.InlineOverlay(false);
			}
			...
			IM.closePrompt = function(node){
				...
				node.find(".inline-processing").hide();
				setTimeout(function(){ node.removeClass('pending-verification'); },400)
		...
		HW.ActionPrompt = new function() {
			...
			*/
			this.init = function(n) {
				...
				cancelBtn = sectionToExpand.find("*[data-widget-action=cancel]").filter(function(){ return sectionToExpand[0] === $(this).closest("*[data-widget-type=action-prompt-target]")[0] });
			...
			*/
			...
			this.reveal = function(e) {
				...
				//var openSection = me.closest('.grey-box').find('[data-widget-type=action-prompt-target].active');
		...
		HW.DynamicFrequency = new function() {
			...
			*/
			this.init = function(node) {
				...
				var recurrenceField = node.find('[name=recurrence]');
				var frequencyField = node.find('[name=frequencyOption]');
				var dateField = node.find('[data-widget-property=date-input]');
				...
				function testToDisplay() {
					var amountValue, frequencyValues, dateValue, frequencyStatement;
					var amountValid = amountField.val().length > 0 ? true : false;
					var dateValid = dateField.val().length > 0 ? true : false;

					amountValue = amountField.val();

					if (oneTimeToggle.length > 0 && oneTimeToggle.is(':checked') && amountValid) {
						...
						dateValues = parseDate(dateField.val());

						frequencyValues = getSelectedRadio(frequencyField);
						frequencyText = frequencyValues['label'];
						frequencyType = frequencyValues['frequencyType'];

						switch (frequencyType) {
							case 'daily' :
							frequencyStatement = amountValue + " will be transferred " + frequencyType + " starting on " + dateValues['thisDay'] + ", " + dateValues['value'] + " and on every day thereafter.  You can edit this any time."
							break;
							case 'weekly' :
							frequencyStatement = amountValue + " will be transferred " + frequencyType + " starting on " + dateValues['thisDay'] + ", " + dateValues['value'] + " and on every " + dateValues['thisDay'] + " thereafter.  You can edit this any time."
							break;
							case 'biweekly' :
							frequencyStatement = amountValue + " will be transferred " + frequencyType + " starting on " + dateValues['thisDay'] + ", " + dateValues['value'] + " and on every other " + dateValues['thisDay'] + " thereafter.  You can edit this any time."
							break;
							case 'monthly':
							frequencyStatement = amountValue + " will be transferred " + frequencyType + " starting on " + dateValues['thisDay'] + ", " + dateValues['value'] + " and on every " + dateValues['thisOrdinal'] + " day of the month thereafter.  You can edit this any time."
							break;
							default:
							frequencyStatement = false;
							break;
						}
						if (frequencyStatement) {
				...
				function getSelectedRadio(nodes) {
					var label, selected, frequencyType;
					$(nodes).each(function() {
						if ($(this).is(':checked')) {
							selected = $(this);
							label = selected.siblings('label').html().toLowerCase();
							frequencyType =  selected.attr('data-frequency');
						}
					});
					return {
						"label" : label,
